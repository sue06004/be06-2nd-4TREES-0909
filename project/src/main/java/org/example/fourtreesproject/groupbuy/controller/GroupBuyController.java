...
public class GroupBuyController {
    ...
    @Operation(summary = "입찰 대기 공구 조회 api",
            description = "현재 입찰 대기 상태인 내 공구 전체 조회.<br><br>" + "※ 일반 회원 로그인이 필요한 기능입니다.")
    @GetMapping("/list/wait")
    public BaseResponse<List<GroupBuyWaitListResponse>> waitList(
            @AuthenticationPrincipal CustomUserDetails customUserDetails
    ) {
        if (customUserDetails.getUser().getRole().equals("ROLE_SELLER")) {
            return new BaseResponse<>(GROUPBUY_LIST_WAIT_EMPTY);
        }
        List<GroupBuyWaitListResponse> result = gpbuyService.waitList(customUserDetails.getUser());
        return new BaseResponse<>(result);
    }
